...
也可以在“历史管理 / 平台模板配置”中维护全平台共用模板：

- 周报模板保存到 `user_data/templates/weekly/template.xlsx`
- 出差报告模板保存到 `user_data/templates/trip/template.docx`

生成文件时所有用户优先使用平台模板。模板需要调整时，可以下载当前模板，用 Excel 或 Word 编辑后重新上传保存。
